import { MenuBarExtra, open, Icon, Color, getPreferenceValues } from "@raycast/api";
...
export default function Command() {
    ...
    const [status, setStatus] = useState<"loading" | "active" | "error" | "unauthorized">("loading");
    ...
    useEffect(() => {
        ...
        async function checkStatus() {
            ...
            const messages: Message[] = [{ role: "user", content: "Ping! Are you there? Just say 'pong' if you are." }];
    ...
    return (
        <MenuBarExtra icon={getIcon()} tooltip="GitHub Copilot Status" isLoading={status === "loading"}>
            <MenuBarExtra.Item
                title={getTitle()}
                subtitle={lastChecked ? `Last checked: ${lastChecked.toLocaleTimeString()}` : ""}
            />
            {errorMessage && (
                <MenuBarExtra.Item title={`Error: ${errorMessage}`} />
            )}
